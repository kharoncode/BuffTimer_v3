...
charactersRoute
	.get('/', async (c) => {
		...
		const resp = await db.select().from(characters).where(eq(characters.id, user.id));
		return c.json(resp);
	})
	.post('/', async (c) => {
		try {
			...
			const resp = await db.insert(characters).values(user).returning();
	...
	.patch('/', async (c) => {
		try {
			...
			const resp = await db.update(characters).set(user).where(eq(characters.id, id)).returning();
